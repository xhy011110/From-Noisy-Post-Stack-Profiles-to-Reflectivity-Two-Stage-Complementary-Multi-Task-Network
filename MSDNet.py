...
MSDNet
Author:Haiyang Xu
Description: This file contains the implementation of MSDNet, including
             Multi-scale Dense Connections (MDC) and Residual Dense Blocks (RDB).
"""
...
class MSDNet(nn.Module):
    """
    MSDNet Architecture.
    Includes Dual Encoders and Dual Decoders with Multi-scale Feature Fusion.
    """
    ...
    def __init__(self, n_classes=2, in_channels=1, is_deconv=True, is_batchnorm=True, res_blocks=18):
        ...
        self.conv_input2 = ConvLayer(in_channels, 4, kernel_size=3, stride=1)
        self.dense0 = nn.Sequential(
            ResidualBlock(16), ResidualBlock(16), ResidualBlock(16)
        )

        # Encoder Stage 1
        self.conv2x = ConvLayer(16, 28, kernel_size=3, stride=2)
        ...
        # Bottleneck / Residual Dehaze Blocks
        ...
        self.conv_output1 = ConvLayer(16, 1, kernel_size=3, stride=1)
        ...
        self.conv_output2 = ConvLayer(16, 2, kernel_size=3, stride=1)

    def forward(self, x1, x2, label_dsp_dim):
        """
        Args:
            x1: Primary input tensor
            x2: Secondary input tensor (concatenated at various stages)
            label_dsp_dim: List or Tuple [H, W] for defining output crop size
        Returns:
            list: [output_decoder1, output_decoder2]
        """
        # --- Encoder Forward ---
        res1x = self.conv_input(x1)
        res1x_1, res1x_2 = res1x.split([(res1x.size()[1] // 2), (res1x.size()[1] // 2)], dim=1)
        feature_mem = [res1x_1]
        x = self.dense0(res1x) + res1x

        res2x = self.conv2x(x)
        res11x = self.conv_input2(x2)
        res2x = torch.cat((res2x, res11x), dim=1)
        ...
        # Preserve encoder features for skip connections
        res8x3 = res8x
        res4x3 = res4x
        res2x3 = res2x
        x3 = x
        ...
        # --- Decoder 1 Forward ---
        res16x_1_dec1 = self.convd16x_1(res16x)
        res16x_1_dec1 = res16x_1_dec1[:, :, 0:res8x.size()[2], 0:res8x.size()[3]]  # Crop to match
        res8x = torch.add(res16x_1_dec1, res8x)
        res8x = self.dense_4_1(res8x) + res8x - res16x_1_dec1
        res8x_1, res8x_2 = res8x.split([(res8x.size()[1] // 2), (res8x.size()[1] // 2)], dim=1)
        ...
        res2x = res2x[:, :, 0:x.size()[2], 0:x.size()[3]]
        x = torch.add(res2x, x)
        x = self.dense_1_1(x) + x - res2x
        x_1, x_2 = x.split([(x.size()[1] // 2), (x.size()[1] // 2)], dim=1)
        x_1 = self.fusion_1_1(x_1, feature_mem_up)
        x_2 = self.conv_1_1(x_2)

        x_final1 = torch.cat((x_1, x_2), dim=1)
        decoder1_output = self.conv_output1(x_final1[:, :, :label_dsp_dim[0], :label_dsp_dim[1]].contiguous())

        # --- Decoder 2 Forward ---
        res16x_2_dec2 = self.convd16x_2(res16x)
        res16x_2_dec2 = res16x_2_dec2[:, :, 0:res8x3.size()[2], 0:res8x3.size()[3]]
        res8x3 = torch.add(res16x_2_dec2, res8x3)
        res8x3 = self.dense_4_2(res8x3) + res8x3 - res16x_2_dec2
        ...
        x_final2 = torch.cat((x_1, x_2), dim=1)
        decoder2_output = self.conv_output2(x_final2[:, :, :label_dsp_dim[0], :label_dsp_dim[1]].contiguous())
...
class LossMSDNet:
    """
    Combined Loss for MSDNet
    """

    def __init__(self, weights=[1, 1], entropy_weight=[1, 1]):
        self.criterion1 = nn.MSELoss()
        # Ensure entropy_weight is on the correct device when initializing the loss
...
if __name__ == '__main__':
    # Test Block
    print("Testing MSDNet...")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Example input tensors
    x1 = torch.randn((2, 1, 256, 256)).to(device)  # Batch size 2
    x2 = torch.randn((2, 1, 256, 256)).to(device)

    # Initialize Model
    model = MSDNet(n_classes=1, in_channels=1, is_deconv=True, is_batchnorm=True).to(device)

    # Forward Pass
    try:
        out = model(x1, x2, label_dsp_dim=[256, 256])
